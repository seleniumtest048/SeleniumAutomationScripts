...
public class DriverScriptTest extends Keywords {
		...
		for (int i = 1; i < tdsheet.getRows(); i++) {
			...
			case "GC":
				if (run.equalsIgnoreCase("Y")) {
					...
					File gc = new File(path
							+ TestConstants.CHROME_BROWSER_DIR_PATH);
			...
			case "IE":
				...
				break;
			case "SAFARI":
				if (browser.equalsIgnoreCase("SAFARI") && run.equalsIgnoreCase("Y")) {
				...
				break;	
			case "BST":
				if (browser.equalsIgnoreCase("BST")
						&& run.equalsIgnoreCase("Y")) {
					...
					browserStack(siteurl, Username, Key, URLKey, Browser,
							Bversion, OS, OsVersion);
	...
	public void controlscript(String browser,Sheet controlshet, Workbook testdatawb)
			throws Exception {
				...
				for (int j = 1; j < tdsheet1.getRows(); j++) {
					...
					if (tcaserunmode.equalsIgnoreCase("y")) {
						...
						if (failcount >= 1 || rptFailCnt >= 1) {
							...
							mainReport(browser,keyword, result, fileName);
							rptFailCnt = 0;
							failcount = 0;
						} else {
							result = "Pass";
	...
	public void browserStack(String siteurl, String username, String key,
			String uRLKey, String browser, String bversion, String oS,
